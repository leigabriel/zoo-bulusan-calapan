import { ChevronDown as ReiconChevronDown, HelpCircle as ReiconHelpCircle, Search as ReiconSearch } from 'reicon-react';
        ...
        <div className="space-y-6">
            <div className="bg-[#f6fdf8] border border-green-300 rounded-3xl p-6 md:p-8">
                <div className="w-14 h-14 bg-green-100 rounded-2xl flex items-center justify-center text-green-800 mb-5">
                    <ReiconHelpCircle className="w-6 h-6" />
                </div>
                <h1 className="text-3xl font-bold text-green-900 mb-2">Admin Help Center</h1>
            ...
            <div className="bg-white border border-green-300 rounded-2xl p-5 md:p-6">
                <div className="relative mb-4">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-green-800">
                        <ReiconSearch className="w-5 h-5" />
                    </div>
                    <input
                        type="text"
                        ...
                        className="w-full bg-[#f6fdf8] border border-green-300 rounded-xl py-3.5 pl-12 pr-4 text-green-900 placeholder-green-700/50 focus:outline-none focus:border-green-400"
                    />
                </div>
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                                    isActive
                                        ...
                                        : 'bg-green-50 text-green-900/70 border-green-300 hover:border-green-400'
                                }`}
                            >
                                {category}
                            </button>
            ...
            <div className="bg-white border border-green-300 rounded-2xl p-5 md:p-6">
                <div className="flex items-center justify-between mb-5">
                    <h2 className="text-xl font-bold text-green-900">How-To Questions</h2>
                    <span className="text-sm text-green-900/60">{filteredFaqs.length} results</span>
                </div>

                {filteredFaqs.length === 0 ? (
                    <div className="bg-green-50 border border-green-300 rounded-xl p-6 text-center text-green-900/60">
                                ...
                                <div key={faq.id} className="border border-green-300 rounded-xl overflow-hidden">
                                    ...
                                    >
                                        <div className="flex items-center justify-between gap-4">
                                            <div>
                                                ...
                                                <p className="text-green-900 font-medium">{faq.question}</p>
                                            </div>
                                            <span className={`text-green-700/60 transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
                                                <ReiconChevronDown className="w-5 h-5" />
                                            </span>
                                        </div>
                                    </button>
                                    ...
                                    {isExpanded && (
                                        <div className="px-4 pb-4">
                                            <div className="bg-green-50 border border-green-300 rounded-lg p-4 text-sm text-green-900/70 leading-relaxed">
                                                {faq.answer}
                                            </div>
                                        </div>
                                    )}
