import { Check as CheckIcon, Scan as ScanIcon, Camera as CameraIcon, Upload as UploadIcon } from 'reicon-react';
            ...
            <div className="flex items-start justify-between gap-4 border-b border-green-100 bg-green-50/50 px-4 py-4 sm:px-6">
                ...
                <div className="flex flex-col-reverse gap-3 border-t border-gray-100 bg-white p-4 sm:flex-row sm:p-6">
                    ...
                    <button
                        ...
                        className={`min-h-12 flex-[2] rounded-xl border-2 px-4 text-sm font-extrabold transition ${scanResult.scanStatus === 'used' || scanResult.scanStatus === 'expired' ? 'cursor-not-allowed border-gray-200 bg-gray-100 text-gray-400' : 'border-green-400 bg-green-400 text-gray-900 hover:bg-green-400'}`}
                    >
                        {loading ? <span className="mx-auto block h-5 w-5 animate-spin rounded-full border-2 border-gray-900/30 border-t-gray-900" /> : scanResult.scanStatus === 'used' ? 'Already checked in' : scanResult.scanStatus === 'expired' ? 'Ticket expired' : <span className="inline-flex items-center justify-center gap-2"><CheckIcon className="h-5 w-5" /><span>Confirm check-in</span></span>}
                    </button>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="mx-auto w-full max-w-5xl space-y-6 p-3 sm:space-y-8 sm:p-6 lg:p-8">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="inline-flex w-fit items-center gap-2 rounded-full border border-green-100 bg-green-50 px-3 py-1.5 text-xs font-bold text-green-700">
            ...
            <div className="w-full overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-xl shadow-gray-200/50 sm:rounded-[2rem]">
                    <div className="flex flex-col gap-4 border-b border-gray-100 bg-gray-50/60 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                        <div className="flex items-center gap-3">
                            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-green-400 text-gray-900 shadow-sm shadow-green-400/20 sm:h-12 sm:w-12">
                                <ScanIcon className="h-7 w-7 sm:h-8 sm:w-8" />
                            </span>
                            <div>
                            ...
                            <div className="flex w-full max-w-xl flex-col items-center justify-center gap-5">
                                ...
                                <div
                                    onClick={() => fileInputRef.current?.click()}
                                     className="group flex aspect-square w-full cursor-pointer flex-col items-center justify-center gap-4 rounded-[1.5rem] border-2 border-dashed border-green-200 bg-green-50/30 p-6 text-center transition-all hover:border-green-400 hover:bg-green-50 sm:rounded-[2rem] sm:border-4 sm:p-12"
                                 >
                                    <div className="mb-1 flex h-16 w-16 items-center justify-center rounded-full bg-white text-green-700 shadow-sm transition-transform group-hover:scale-105 sm:h-24 sm:w-24">
                                        <UploadIcon className="w-8 h-8" />
                                    </div>
                                    <div>
