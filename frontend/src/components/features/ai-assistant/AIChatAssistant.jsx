                                        ...
                                        return (
                                            <div key={`${message.role}-${index}`} className={`flex min-w-0 gap-2.5 sm:gap-3 ${isUser ? 'flex-row-reverse' : 'items-end'}`}>
                                                ...
                                                <div className={`min-w-0 flex-1 flex flex-col gap-2.5 ${isUser ? 'items-end' : 'items-start'}`}>
                                                    ...
                                                    <div className="max-w-[88%] sm:max-w-[80%] px-4 sm:px-5 py-3.5 sm:py-4 text-sm sm:text-base leading-relaxed font-medium whitespace-pre-wrap break-words [overflow-wrap:anywhere]" style={isUser ? { background: THEME.accentDark, color: '#142018', borderRadius: '1rem 1rem 0.25rem 1rem' } : { background: THEME.surface, color: THEME.text, border: `1px solid ${THEME.border}`, borderRadius: '0.25rem 1rem 1rem 1rem', boxShadow: '0 2px 8px rgba(15,23,42,0.04)' }}>{message.content}</div>
