...
const AccountDetailsModal = ({ isOpen, onClose, role, profile, previewImage, fileInputRef, imageUploading, onUploadImage, onSaveDetails, profileSaving, onPassword, onChangeEmail, emailSaving = false, onMasterKey, onToggleMasterKey, masterKeyLabel = 'Master Key', masterKeyConfigured = false, masterKeyEnabled = false }) => {
    ...
    const save = async () => {
        const saved = await onSaveDetails(form);
        if (saved !== false) setEditing(false);
    };

    const openEmailEditor = () => {
        setEmailForm({ email: profile.email || '', currentPassword: '' });
        setEditingEmail(true);
    };

    const saveEmail = async () => {
        const saved = await onChangeEmail(emailForm);
        if (saved !== false) setEditingEmail(false);
    };
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4 backdrop-blur-sm">
                <div className="w-full max-w-2xl overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-xl">
                    ...
                    <div className="space-y-5 p-5">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={onPassword} className="rounded-xl border border-emerald-300 px-4 py-3 font-semibold text-emerald-700 hover:bg-emerald-50">Change password</button><button type="button" onClick={openEmailEditor} className="rounded-xl border border-emerald-300 px-4 py-3 font-semibold text-emerald-700 hover:bg-emerald-50">Change email</button>{role === 'Admin' && <button type="button" onClick={onMasterKey} className="rounded-xl border border-emerald-300 px-4 py-3 font-semibold text-emerald-700 hover:bg-emerald-50">{masterKeyLabel}</button>}{role === 'Admin' && masterKeyConfigured && <button type="button" onClick={onToggleMasterKey} className="rounded-xl border border-amber-300 px-4 py-3 font-semibold text-amber-700 hover:bg-amber-50">{masterKeyEnabled ? 'Disable Master Key' : 'Enable Master Key'}</button>}<button type="button" onClick={openEditor} className="rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white hover:bg-emerald-700 sm:col-span-2">Change details</button></div>
                    </div>
                </div>
            </div>
            {editing && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-gray-900/55 p-4 backdrop-blur-sm"><div className="w-full max-w-xl rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-gray-200 p-5"><h2 className="text-xl font-bold text-gray-900">Change details</h2><button onClick={() => setEditing(false)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><CloseCircle size={20} /></button></div><div className="space-y-4 p-5"><input value={form.firstName} onChange={e => setForm({ ...form, firstName: sanitizeInput(e.target.value) })} placeholder="First name" className="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 outline-none focus:border-emerald-500" /><input value={form.lastName} onChange={e => setForm({ ...form, lastName: sanitizeInput(e.target.value) })} placeholder="Last name" className="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 outline-none focus:border-emerald-500" /><div><input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={onUploadImage} /><button type="button" disabled={imageUploading} onClick={() => fileInputRef.current?.click()} className="text-sm font-semibold text-emerald-700">{imageUploading ? 'Uploading...' : 'Change profile photo'}</button></div><div className="flex justify-end gap-3"><button onClick={() => setEditing(false)} className="rounded-xl px-4 py-3 text-gray-600 hover:bg-gray-100">Cancel</button><button onClick={save} disabled={profileSaving} className="rounded-xl bg-emerald-600 px-5 py-3 font-semibold text-white disabled:opacity-50">{profileSaving ? 'Saving...' : 'Save details'}</button></div></div></div></div>}
            {editingEmail && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-gray-900/55 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-gray-200 p-5"><div><h2 className="text-xl font-bold text-gray-900">Change email address</h2><p className="mt-1 text-sm text-gray-500">The new address must be verified before your next login.</p></div><button onClick={() => setEditingEmail(false)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><CloseCircle size={20} /></button></div><div className="space-y-4 p-5"><div><label className="mb-2 block text-sm font-semibold text-gray-700">New email address</label><input type="email" value={emailForm.email} onChange={e => setEmailForm({ ...emailForm, email: sanitizeEmail(e.target.value) })} className="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 outline-none focus:border-emerald-500" /></div><div><label className="mb-2 block text-sm font-semibold text-gray-700">Current password</label><PasswordInput value={emailForm.currentPassword} onChange={e => setEmailForm({ ...emailForm, currentPassword: e.target.value })} className="rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 outline-none focus:border-emerald-500" /></div><div className="flex justify-end gap-3"><button onClick={() => setEditingEmail(false)} className="rounded-xl px-4 py-3 text-gray-600 hover:bg-gray-100">Cancel</button><button onClick={saveEmail} disabled={emailSaving || !emailForm.email || !emailForm.currentPassword} className="rounded-xl bg-emerald-600 px-5 py-3 font-semibold text-white disabled:opacity-50">{emailSaving ? 'Sending verification...' : 'Change email'}</button></div></div></div></div>}
        </>
    );
};
